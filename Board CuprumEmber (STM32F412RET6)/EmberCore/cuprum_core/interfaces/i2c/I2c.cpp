...
namespace I2c {
  ...
  uint8_t send(I2cStruct *i2c, uint8_t adress, uint8_t data) {
    ...
    *i2c->DR = data;  // Transmit Data byte
    ...
    while (readBit(i2c->SR1, I2c::TXE_MSK) == 0) {
      ...
      if(readBit(i2c->SR1, I2c::ACK_MSK == 1)) {
  ...
  uint8_t readPackage(I2cStruct *i2c, uint8_t adress, uint8_t readableRegister, uint8_t* buffer, uint8_t size) {
    ...
    *i2c->SR2;                    // Clearing the ADDR bit by reading SR1 register
    *i2c->DR = readableRegister;  // Transmit Data byte
    ...
    *i2c->CR1 |= I2c::START_MSK;  // Restart I2c transmit
    
    SysTick::reset();
    while (!(*i2c->SR1 & SB_MSK)) {
      if(SysTick::getTick() > I2c::TIMEOUT_STANDART) {
        *i2c->CR1 |= I2c::STOP_MSK;   // Stop transmit
      }
    }

    *i2c->CR1 |= ACK_MSK; // Enable ACK signal from MCU
    
    *i2c->DR = (adress << 1) | 0x01;      // Send device adress wuth read mode
    SysTick::reset();
    while (readBit(i2c->SR1, I2c::ADDR_MSK) == 0) {
      if(SysTick::getTick() > I2c::TIMEOUT_STANDART){
        *i2c->CR1 |= I2c::STOP_MSK;   // Stop transmit
